  ...
  */
   ...
   */
		...
		_this.on('component.removed', function (componentName, data) {
			// Package <> Component relationship
			_this.removePackageComponent(data.packageId, componentName);
			// Component <> Demo relationship
			_this.remove(_this.componentDemosRef(componentName));
		});
  ...
  */
   ...
   */
		...
		value: function setPackage(name, data) {
			...
			if (!data) {
				return this.getPackage(name).then(function (pkg) {
					return _this3.set(path, data).then(function () {
						_this3.emit('package.removed', name, pkg);
					});
				});
			}
			return this.set(path, data).then(function () {
				_this3.emit('package.added', name, data);
			});
		}
		...
		value: function getPackages() {
			...
			return this.get('packages').then(function (snapshot) {
				...
				packages.map(function (pkg) {
					queue.push(_this4.getPackageComponentsCount(pkg.name).then(function (count) {
   ...
   */
		...
		value: function removePackageComponent(packageName, componentName) {
			...
			if (!data) {
				return this.getComponent(name).then(function (_ref) {
					...
					return _this5.set(path, data).then(function () {
						_this5.emit('component.removed', name, component);
					});
				});
			}
			return this.set(path, data).then(function () {
				_this5.emit('component.added', name, data);
			});
		}
   ...
   */
		...
		value: function removeComponent(name) {
			return Promise.all([this.setComponent(name, null), this.setComponent(name, null, true)]);
		}
   ...
   */
		...
		value: function getPackageComponentsCount(packageName) {
   ...
   */
		...
		value: function removePackageComponents(packageName) {
			var _this7 = this;
			...
			return this.get(this.getPackageComponentsRef(packageName)).then(function (snapshot) {
				...
				_this7.emit('serviceLoading');
				return Promise.all(removeComponents);
			}).then(function () {
				_this7.emit('serviceComplete');
			}).catch(function (e) {
				_this7.emit('serviceError', e);
			});
		}
		...
		value: function setPage(data, pageId) {
			...
			if (!data) {
				return this.getPage(pageId).then(function (page) {
					return _this10.set(ref, data).then(function () {
						_this10.emit('page.removed', pageId, page);
					});
				});
			}
			return this.set(ref, data).then(function () {
				_this10.emit('page.added', pageId, data);
				return pageId;
			});
		}
   ...
   */
		...
		key: 'removePages',
		value: function removePages(packageName) {
			var _this13 = this;

			return this.get(this.getPackagePagesRef(packageName)).then(function (snapshot) {
				var pageIds = Object.keys(snapshot.val());
				return Promise.all(pageIds.map(function (pageId) {
					return _this13.removePage(pageId);
				}));
			});
		}
   ...
   */
		...
		value: function removePackagePage(packageName, pageId) {
